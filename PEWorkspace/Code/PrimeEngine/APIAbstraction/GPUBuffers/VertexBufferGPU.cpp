...
void VertexBufferGPU::createGPUBufferFromSource_DetailedSkin(PositionBufferCPU &vb, TexCoordBufferCPU &tcb, TexCoordBufferCPU &wb, SkinWeightsCPU &weights, NormalBufferCPU &nb, TangentBufferCPU &tb)
{
    int vertexScalarSize = 3 + 4 + 4 + 2 + 3 + 3;
	if (DEFAULT_SKIN_WEIGHTS_PER_VERTEX == 4)
	{
	...
	PrimitiveTypes::UInt32 itcval = 0;
	PrimitiveTypes::UInt32 itcvalWind = 0;

	for (PrimitiveTypes::UInt32 iv = 0; iv < vb.m_values.m_size / 3; iv++)
	{
		PrimitiveTypes::Float32 x,y,z, u, v, nx, ny, nz, tx, ty, tz, w1, w2;
		PrimitiveTypes::UInt32 curInex = iv * 3;
		x = vb.m_values[curInex]; y = vb.m_values[curInex + 1]; z = vb.m_values[curInex + 2];
		...
		res.m_values.add(w1); res.m_values.add(w2);
	}
...
void VertexBufferGPU::createGPUBufferFromSource_DetailedMesh(PositionBufferCPU &vb, TexCoordBufferCPU &tcb, TexCoordBufferCPU &wb, NormalBufferCPU &nb, TangentBufferCPU &tb)
{
//OGL is still using old way of forwarding all source buffers to low level helper implementation
#if APIABSTRACTION_OGL
	memset(m_bufs, 0, sizeof(m_bufs));
